...
%%
% Book metadata
\title{Building Test Collections for Information Retrieval Experiments}
...
\newcommand{\todo}[1]{\marginnote{\hl{TODO #1}}}
...
Look out for interface features (or misfeatures) that can bias relevance judgments towards particular systems, retrieval models, or document types.  We sorted pools by document identifier to avoid bias towards systems, but that assumed that those identifiers were not confounded with relevance at all.  Better sorts are available that support consistency; see below.  \todo{I think Tetsuya has a very recent paper that shows the advantage of score-sorting the pool, without bias.}

In my experience, assessors like the ability to go back and change an assessment they made.  As such, our assessment tools present the pool as a list they can freely navigate.  Some assessment optimization procedures like feedback-driven assessment assume no backtracking, where a better design would be to retain judgment states and allow the assessor to make corrections.

The interface should not allow the assessor make an error, and should look out for possible errors.  For example, since our pool is a list, it's easy to accidentally skip a document.  As a result, the interface warns them if they leave a pool with unjudged documents, and has list filters to narrow the view to unjudged or relevant documents only.
...
Assessors will do a better job of assessing, and feel more confident in their work, if the assessment setup supports consistency.  Consistency is making similar relevance judgments about similar documents, and also the ability to recognize when that similarity loses relevance.

Consistency is difficult because many documents may be similar to each other, but they still might not be equally relevant.  One example of this is a set of pages from the same website.  Each page will include a number of identical or similar elements, such as navbars, breadcrumbs, tables of contents, headers and footers, and in fact only a small part of each page may be devoted to the actual content of that page.  Shingling and other algorithms to detect near-duplicates may cluster all these pages together, but it might be the small but critical central text on a page that makes that page relevant.  Recall the TREC relevance criterion: even a single relevant sentence is enough to judge a document relevant.
